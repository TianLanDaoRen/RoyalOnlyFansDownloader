...
class WanWanScraper:
    ...
    async def process_api_response(self, response: Response):
        try:
            ...
            if f"/users/{self.user_id}/posts" in response.url and response.status == 200:
                ...
                if "hasMore" in data: self.has_more = data["hasMore"]
                if "list" in data:
                    for post in data["list"]:
                        if "media" not in post: continue
                        post_id = post.get("id")
                        text = post.get("text", "")
                        ...
                        for media in post["media"]:
                            ...
                            source = None
                            if m_type == "photo":
                                source = media.get("files", {}).get("full", {}).get("url")
                            elif m_type == "video":
                                files = media.get("files", {})
                                source = files.get("full", {}).get("url")
                                if not source:
                                    v_src = media.get("videoSources", {})
                                    source = v_src.get("720") or v_src.get("240")
                            
                            if source and source not in self.assets:
                                asset = Asset(post_id, text, source, m_type, posted_at, source_type=source_type)
    ...
    def save_cache(self):
        try:
            with open(f"{CACHE_PREFIX}{self.user_id}.json", "w", encoding="utf-8") as f:
                json.dump({url: asset.to_dict() for url, asset in self.assets.items()}, f, indent=4, ensure_ascii=False)
        except: pass
            ...
            if Confirm.ask(f"[bold green]发现本地缓存 ({cache_file})，是否使用？[/bold green]"):
                try:
                    ...
                    for url, item in data.items():
                        ...
                        if self.download_mode == "photos" and asset.media_type != "photo": continue
                        if self.download_mode == "videos" and asset.media_type != "video": continue
    ...
    def archive_manifest(self):
        try:
            with open(self.base_dir / f"manifest_{time.strftime('%Y%m%d_%H%M%S')}.json", "w", encoding="utf-8") as f:
                json.dump([asset.to_dict() for asset in self.assets.values()], f, indent=4, ensure_ascii=False)
        except: pass
    ...
    async def _scroll_page(self, page, description="抓取中"):
        ...
        with console.status(f"[bold magenta]琬琬正在{description}...[/bold magenta]") as status:
            while True:
                ...
                status.update(f"[bold magenta]🚀 {description}... 已捕获: {len(self.assets)}[/bold magenta]")

                if new_height == last_height:
                    no_change_count += 1
                    if no_change_count >= 4:
                        if not self.has_more:
                            console.log(f"[green]{description} - API 提示到底。[/green]")
                            break
                        else:
                    ...
                    if no_change_count >= 6:
                        console.log(f"[yellow]{description} - 高度不再变化，停止。[/yellow]")
                        break
                else:
                    no_change_count = 0
    ...
    async def start_scraping(self):
        async with async_playwright() as p:
            ...
            rprint(Panel.fit("[bold yellow]浏览器已启动！请登录...[/bold yellow]", title="👑 皇上请操作"))
            ...
            if self.download_mode != "all":
                ...
                for url, asset in self.assets.items():
                    if self.download_mode == "photos" and asset.media_type == "photo": filtered[url] = asset
                    elif self.download_mode == "videos" and asset.media_type == "video": filtered[url] = asset
    ...
    async def download_worker(self, client: httpx.AsyncClient, asset: Asset, progress: Progress, main_task_id: TaskID, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            progress.update(main_task_id, description=f"{desc}{asset.filename}...")
            ...
            try:
                async with client.stream("GET", asset.url, headers=headers, follow_redirects=True) as response:
                    if response.status_code != 200:
                        progress.console.print(f"[bold red]❌ 失败: {asset.filename}[/bold red]")
                        progress.advance(main_task_id)
                        return
    ...
    async def download_all(self):
        ...
        rprint(f"[bold cyan]🚀 准备下载 {len(self.assets)} 个文件 (包含挖坟所得)...[/bold cyan]")
        ...
        else: client_kwargs["trust_env"] = True 
        ...
        async with httpx.AsyncClient(**client_kwargs) as client:
            with Progress(
                ...
                main_task = progress.add_task(f"[bold yellow]总进度[/bold yellow]", total=len(self.assets))
                tasks = []
                for asset in self.assets.values():
                    tasks.append(self.download_worker(client, asset, progress, main_task, semaphore))
                await asyncio.gather(*tasks)
        
        self.archive_manifest()
        rprint(Panel(f"[bold green]所有任务（含挖坟）圆满完成！[/bold green]", border_style="green"))

# --- 辅助函数 (保持不变) ---
def get_valid_user_id() -> str:
    while True:
        uid = Prompt.ask("[cyan]请输入博主的 ID (纯数字)[/cyan]", default="76031078")
        if uid.strip().isdigit(): return uid.strip()
...
def get_auth_data():
    ...
    if config and "email" in config:
        if Confirm.ask(f"[green]发现已保存账号 ({config['email']})，是否使用？[/green]", default=True): data = config
        else: data = {}
    ...
    if config and "proxy" in config:
        if Confirm.ask(f"[green]发现已保存代理 ({config['proxy']})，是否使用？[/green]", default=True): proxy = config["proxy"]
    ...
    if Confirm.ask("是否更新本地配置？", default=True):
        ...
        if proxy: new_config["proxy"] = proxy
...
def main():
    ...
    rprint(Panel.fit("[bold magenta]💖 琬琬 OnlyFans 专属下载器 (摸金校尉版) 💖[/bold magenta]", border_style="magenta"))
    ...
    try:
        if scraper.load_cache(): asyncio.run(scraper.download_all())
        else: asyncio.run(scraper.start_scraping())
